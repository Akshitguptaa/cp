...
int func(int n,int x,int y){
    ...
    if(x<=l && y<=l){
        ...
        return ((int)1 <<temp)+func(n-1, x-l, y-l);
    }else if(x>l && y<=l) {
        return 2*((int)1 << temp)+ func(n-1, x-l, y);
            }
    
                return 3 * ((int)1<<temp) + func(n-1, x, y-l);
}

vi func1(int n,int z){
    if(n==1){
        if(z==1){
            return {1,1};
        }
    ...
    if(z<=sq){
        ...
        auto a1= func1(n-1,z-2*sq);
        return {a1[0]+l,a1[1]};

    }else{
        auto a1= func1(n-1,z-3*sq);
        return {a1[0],a1[1]+l};
