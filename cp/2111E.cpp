...
void solve(){
    ...
    for(auto &i:s){
        if(i=='b'){
            if(!ba.empty()){
                i='a';
                ba.erase(ba.find_by_order(0));
                // direct
                continue;
            }
            if(!bc.empty()){
                auto low= *bc.find_by_order(0);
                auto it= ca.upper_bound(low);
                
                if(it!=ca.end()){
                    i = 'a';
                    bc.erase(bc.find_by_order(0));
                    ca.erase(it);
                    // indirect
                }
                continue;
        ...
        if(i=='c'){
            if(!ca.empty()){
                i= 'a';
                ca.erase(ca.find_by_order(0));
                continue;
            }
            if(!cb.empty()){
                auto low= *cb.find_by_order(0);
                auto it= ba.upper_bound(low);
        
                if(it!=ba.end()){
                    i = 'a';
                    cb.erase(cb.find_by_order(0));
                    ba.erase(it);
                }else{
                    i= 'b';
                    cb.erase(cb.find_by_order(0));
