...
void func(int &x,set<int> &st1,set<int> &st2){
    ...
    if(x>0){
        ...
        for(int i=1;i*i<=(x);i++){
            if(x%i==0){
                ...
                if(st1.count(temp)&&st2.count(temp1)){
                    yes();
                    return ;
                 ...
                 if(st1.count(-temp1)&& st2.count(-temp)){
                    yes();
                    return ;
                }
                 if(st1.count(temp1)&& st2.count(temp)){
...
void solve(){
    ...
    // beauty s1.s2
    int s1=accumulate(all(v),(int)0);
    int s2=accumulate(all(v1),(int)0);
    set<int> st1,st2;
    for(int i=0;i<n;i++){
        st1.insert(s1-v[i]);
    }
