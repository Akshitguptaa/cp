...
pii func(int l,int r,vi& one, vvi& pre){
    auto cnt= upper_bound(all(one), r)- lower_bound(all(one),l);

    if(!cnt){
        return {0,0};
    }

    int st = lower_bound(all(one),l)-one.begin();

    int rem = st % 3;
    int res= pre[rem][(st+cnt- rem) /3 ] - pre[rem][(st-rem) / 3];
...
void solve(){
    ...
    while(q--){
        ...
        // auto [c0,val0]= func(l,r,zero,pre0);
        // auto [c1,val1]= func(l,r,one,pre1);

        int c1= (one[r]-one[l-1]);
        int c0= r-l+1-c1;
