...
vector<int> getres(int l, int r) {
    ...
    candidates.pb(l);
    if (l + 1 <= r) candidates.pb(l + 1);
    if (l + 2 <= r) candidates.pb(l + 2);
    if (r - 2 >= l) candidates.pb(r - 2);
    if (r - 1 >= l) candidates.pb(r - 1);
    candidates.pb(r);
...
void solve() {
    ...
    for (int i = 0; i < n; ++i) {
        for (int j = i + 1; j < n; ++j) {
            for (int k = j + 1; k < n; ++k) {
                int temp = func(res[i], res[j], res[k]);
                if (temp > maxx) {
                    maxx = temp;
                    a = res[i];
                    b = res[j];
