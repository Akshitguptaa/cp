...
typedef tree<pair<int,int>, null_type, less<pair<int,int>>, rb_tree_tag,tree_order_statistics_node_update> ordered_set; //m - eq // order_of_key(k) { <k } // *find_by_order(i) { 0-based }
...
void solve(){
    ...
    for(int i=n;i>=1;i--){
        int temp = min(n,4*i-1);

        int cnt= st.order_of_key({temp+1,0});
        if(2*i <= temp){
            int val= st.order_of_key({2*i+1,0})- st.order_of_key({2*i,0});
            cnt-=val;
        }
        
        if(3*i <=temp){
            int val= st.order_of_key({3*i+1,0})- st.order_of_key({3*i,0});
            cnt-=val;
            
        }
        
        int val= st.order_of_key({i+1,0})- st.order_of_key({i,0});
        cnt-= val;
