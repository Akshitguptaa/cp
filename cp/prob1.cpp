...
bool func(vi& st,vi& temp,int curr,int maxx,map<vi,int>& mp1) {
    int n= st.size();
    if(st==temp){
        return 1;
    }
    ...
    for(int i=0; i<n;i++){
        for(int j=i; j<n;j++){
            for(int k=0; k<=(n-(j-i+1)); k++){ 
                ...
                for(int id=k; id<rem.size(); id++){
                    vec.pb(rem[id]);
                }
                
                if(func(vec,temp, curr+1, maxx, mp1)){
                    return 1;
                }
            }
        }
