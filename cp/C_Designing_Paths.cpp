...
// void dfs(int i,int d,vi& dist,vvi& v, const vector<vpii>& vec, int K) {
    ...
    while(!q.empty()){
        ...
        for(auto& p : vec[u]){
            ...
            while(it != st[x].end()){
                ...
                for(auto& [a,b] : vec[val]){
                    st[a].erase(b);
                }

                it = st[x].upper_bound(temp);
            }
        }
    }
