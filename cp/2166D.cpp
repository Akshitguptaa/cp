...
int func(int idx,int sum){
    ...
    int val = cnt[idx];
    int pick = func(idx+1,sum-val);
    pick = mod_mul(val,pick,mod);

    int npick = func(idx+1,sum);

    vec[idx][sum]= (pick+npick)%mod;
...
void solve(){
    ...
    int maxx=*max_element(all(cnt));    
    
    vec= vvi(n+1,vi(n+1,-1));

    int ans=0;
    while(maxx<=n){
        ans= (ans+ func(0,maxx))%mod;
        maxx++;
    }
