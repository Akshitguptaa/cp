...
int func(int i,vi &next,vi &nums,vi &dp){
    if(dp[i]!=-1){
        return dp[i];
    }

    dp[i]=nums[i]+func(next[i], next, nums, dp);
    dp[i]= max(dp[i],func(i+1,next,nums,dp));
    return dp[i];
}
...
void solve(){
    ...
    for(int i=0;i<n;i++){
        ...
        int temp = max(i+1,next[i]);
        l=max(l,temp);
        next[i] = l;
    }
    int maxx = 0;
    for(int i=0;i<n;i++){
        maxx=max(maxx,func(i,next,nums,dp));
    }
    cout<<maxx<<endl;
