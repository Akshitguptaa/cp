...
void solve(){
    ...
    for(int i=0;i<n;i++){
        ...
        if(x==0){
            temp[0]= (temp[0]+ dp2[0]+dp1[0])%MOD;
            temp[2]= (temp[2]+dp2[3]+dp1[1])%MOD;
        }else{
            temp[1]= (temp[1]+ dp2[2]+ dp1[0])%MOD;
            temp[3]= (temp[3]+ dp2[1]+ dp1[1])%MOD;
        }
     ...
     int ans= (accumulate(all(dp2),(int)0)) % MOD;
